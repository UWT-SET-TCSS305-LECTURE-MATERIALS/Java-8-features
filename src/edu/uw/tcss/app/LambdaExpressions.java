...
public final class LambdaExpressions {
    ...
    private static void doMath(final int theNumberOfProblems,
                        final IntSupplier theOperands,
                        final String theOpString,
                        final IntBinaryOperator theOperator,
                        final IntSupplier theInput,
                        final Consumer<String> theOutput) {

        int numberCorrect = 0;
        for (int i = 0; i < theNumberOfProblems; i++) {
            final int x = theOperands.getAsInt();
            final int y = theOperands.getAsInt();
            theOutput.accept(x + " " + theOpString + " " + y + " = ");

            final int answer = theOperator.applyAsInt(x, y);
            final int response = theInput.getAsInt();
...
//        
//        doMath(NUMBER_OF_PROBLEMS -1 , 
//               LambdaExpressions::getRando, 
//               "*", 
//               SimpleMathOperations::multiply,
//               scan::nextInt,
//               LambdaExpressions::logMessage);  
        
        String[] names = {"abc", "xyz", "lmnop", "def" };
        Stream.of(names).forEach(System.out::println);
        bubbleSort(names, (x, y) -> x.compareTo(y) < 0);
        Stream.of(names).forEach(System.out::println);
        
        Integer[] nums = {1, 5, 7, 3, 5, 2, 8, -9};
   ...
   public static int getRando() {
    ...
    public static <T> void bubbleSort(final T theArray[], final BiPredicate<T, T> pred) {
        int n = theArray.length;
        for (int i = 0; i < n - 1; i++) {
            for (int j = 0; j < n - i - 1; j++) {
                if (pred.test(theArray[j], theArray[j + 1])) {
                    // swap temp and arr[i]
                    T temp = theArray[j];
